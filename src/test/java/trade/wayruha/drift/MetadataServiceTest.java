...
//    final PlaceOrderRequest placeOrderRequest = new PlaceOrderRequest(List.of(PlaceOrderParams.SpotOrder(0, BigDecimal.valueOf(0.1), BigDecimal.valueOf(150), false, OrderType.LIMIT, 101, false, 0)));
    final PlaceOrderRequest placeOrderRequest = new PlaceOrderRequest(List.of(PlaceOrderParams.PerpOrder(0, BigDecimal.valueOf(0.2), BigDecimal.valueOf(120), false, OrderType.LIMIT,101)));
    final String tx = httpGatewayService.placeOrder(placeOrderRequest);
    System.out.println("tx: " + tx);
  }

  private static void cancelAllOrders(){
    final String tx = httpGatewayService.cancelAllOrders();
    System.out.println("tx: " + tx);
  }
  ...
  private static void getAllMarketPositions(){
    final BaseMarketResponse<MarketPositionItem> marketPositions = httpGatewayService.getAllMarketPositions();
